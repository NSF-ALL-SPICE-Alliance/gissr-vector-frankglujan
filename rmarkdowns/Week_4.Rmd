...
```{r Challenge 1}
# Guam  coordinate boundaries 

inat_obs_df_gu <- get_inat_obs(taxon_id = 504034,
                               place_id = 7316, 
                               quality = "research",
                               geo = TRUE,
                               maxresults = 100)

save(inat_obs_df_gu, file = "guam_torchwood_inat.Rdata")

inat_obs_sf_gu <- inat_obs_df_gu %>%
  select(longitude, latitude, datetime, common_name, scientific_name, image_url, user_login) %>% 
  st_as_sf(coords = c("longitude", "latitude"), crs = 4326)
...
ggplot() + geom_point(data = inat_obs_df_gu, aes(x= longitude, y = latitude, color = common_name, text = common_name))
...
p <- ggplot() +
  ...
  geom_point(data = inat_obs_df_gu, aes(x = longitude, y = latitude, color = common_name, text = common_name)) + # here is our iNaturalist data
  ...
  ggtitle("Cycads on Guam", subtitle = "Based on iNaturalist Data as of October 2024") +
  theme_bw() +
  labs(
    color="Common Name", #this changes the title of our legend
    x = "Longitude",
...
inat_obs_popup_sf_guam <- inat_obs_sf_gu %>% 
  mutate(popup_html = paste0("<p><b>", common_name, "</b><br/>",
                             "<i>", scientific_name, "</i></p>",
                             "<p>Observed: ", datetime, "<br/>",
                             "User: ", user_login, "</p>",
...
htmltools::p("iNaturalist Observations of Cycads of Guam",
             htmltools::br(),
             inat_obs_popup_sf_guam$datetime %>% 
               as.Date() %>% 
               range(na.rm = TRUE) %>% 
               paste(collapse = " to "),
             style = "font-weight:bold; font-size:110%;")

leaflet(inat_obs_sf_gu) %>% 
  setView(lng = 144.6, lat = 13,2 , zoom = 12)%>%
  addTiles() %>% 
  addCircleMarkers(data = inat_obs_popup_sf_guam,
                   popup = ~popup_html, 
                   radius = 5)
