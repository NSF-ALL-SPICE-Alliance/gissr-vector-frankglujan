...
ggplot() +
  geom_sf(data = gu_schools$osm_polygons) +
  geom_sf_text(data = gu_schools$osm_polygons, aes(label = name), size = 2, hjust = 0) +
  coord_sf(xlim = c(144.61832, 144.95696), ylim = c(13.24599, 13.65438), expand = FALSE)
...
# data for small streets on Guam
gu_small_streets <- gu_bb %>%
  opq() %>%
  add_osm_feature(key = "highway", value = c("residential", "living_street","unclassified", "service","footway")) %>%
  osmdata_sf()
...
# visualising all retrieved features over each other to form a map of Maui
ggplot() +
  geom_sf(data = gu_streets$osm_lines, inherit.aes = FALSE, color = "#ffbe7f", size = .4, alpha = .8) +
  geom_sf(data = gu_small_streets$osm_lines, inherit.aes = FALSE, color = "#a6a6a6", size = .2, alpha = .8) +
  geom_sf(data = gu_coastline$osm_lines, inherit.aes = FALSE, color = "black", size = .8, alpha = .5) +
  geom_sf(data = gu_schools$osm_polygons, inherit.aes = FALSE, colour = "red", fill = "red", alpha = .5, size = 1) +
  coord_sf(xlim = c(144.61832, 144.95696), ylim = c(13.24599, 13.65438), expand = TRUE) +
  geom_sf_text(data = gu_schools$osm_polygons, aes(label = name), size = 2, hjust = 0) +
  theme_bw() +
  labs(
    title = "Schools on Guam",
    x = "Longitude",
    y = "Latitude"
  )
...
leaflet() %>%
  addTiles() %>%
  addPolygons(
    data = gu_schools$osm_polygons,
    label = gu_schools$osm_polygons$name
  )
